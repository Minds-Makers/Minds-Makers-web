...
const DEFAULT_CONTACT = {
  ...
  serviceTypes: [
    { en: 'NEXORA — AI Testing & QA Automation', ar: 'نيكسورا — اختبار وأتمتة ضمان الجودة' },
...
export default function Contact() {
  ...
  const { data } = useData()
  const s = data.site
  const c = data.contact || DEFAULT_CONTACT
    ...
    <>
      <section className="hero" style={{ padding: '80px 0 50px' }}>
        <div className="container">
          ...
          <h1 style={{ marginTop: 18, fontSize: 'clamp(30px,4vw,46px)', color: '#fff', maxWidth: '18ch' }}>{t(c.hero.title)}</h1>
      ...
      <section className="section" style={{ paddingTop: 0 }}>
        <div className="container">
          <div style={{ display: 'flex', gap: 4, marginBottom: 40, borderBottom: '1px solid var(--line)' }}>
            {[{ id: 'request', label: c.tabs.request }, { id: 'contact', label: c.tabs.general }].map(tb => (
...
function ServiceRequestForm({ t, c, siteEmail }) {
  ...
  const fields = c.fields || DEFAULT_CONTACT.fields
  const showF = (key) => fields[key]?.enabled !== false
  ...
  if (status === 'success') return (
    <div style={{ padding: '40px 32px', textAlign: 'center', background: 'rgba(79,216,255,.04)', border: '1px solid rgba(79,216,255,.15)', borderRadius: 'var(--r-lg)' }}>
      ...
      <button className="btn btn-ghost" style={{ marginTop: 24 }} onClick={() => setStatus(null)}>{t({ en: 'Submit another request', ar: 'إرسال طلب آخر' })}</button>
    </div>
  )
  ...
  return (
    <form className="form" onSubmit={handleSubmit}>
      ...
      {showF('description') && <div><label className="field-label">{t(fields.description?.label)}{fields.description?.required && ' *'}</label><textarea className="field-textarea" style={{ minHeight: 140 }} value={form.description} onChange={e => set('description', e.target.value)} required={fields.description?.required} placeholder={t({ en: "Tell us what you need…", ar: "احكيلنا إيه اللي محتاجه…" })} /></div>}
  ...
  return (
    <div className="contact-info">
      <div className="contact-item">
        <div className="contact-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg></div>
        <div><h4>{t({ en: 'Email us', ar: 'راسلنا' })}</h4><p><a href={`mailto:${s.email}`} style={{ color: '#fff' }}>{s.email}</a></p></div>
      </div>
      <div className="contact-item">
        <div className="contact-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg></div>
        <div><h4>LinkedIn</h4><p><a href={s.linkedin} target="_blank" rel="noopener" style={{ color: '#fff' }}>Minds Makers</a></p></div>
      </div>
      <div className="contact-item">
        <div className="contact-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg></div>
        <div><h4>{t({ en: 'Location', ar: 'الموقع' })}</h4><p style={{ color: '#fff' }}>{t(s.location)}</p></div>
      </div>
      <div style={{ padding: '16px 20px', background: 'rgba(79,216,255,.04)', border: '1px solid rgba(79,216,255,.12)', borderRadius: 'var(--r-md)', fontSize: 13 }}>
        <strong style={{ color: '#fff', display: 'block', marginBottom: 6 }}>{t({ en: 'Response time', ar: 'وقت الرد' })}</strong>
        {t(c.responseTime)}
      </div>
    </div>
  )
